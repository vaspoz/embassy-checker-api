...
module.exports = (
	...
	(async () => {
		...
		const magicNumbersPath = path.join(
			__dirname,
			"images/magicNumbers.png"
		);
		const tempScreenShot = path.join(__dirname, "images/screenshot.png");
		...
		await page.evaluate(
			(a, b) => {
				...
				document.querySelector("#ctl00_MainContent_txtUniqueID").value =
					b;
			},
			input_id,
			input_code
...
const setMagicNumbers = (
	...
	return (number) => {
		...
		(async () => {
			...
			console.log(
				"We are in the next page already. Selecting first checkbox"
			);
			result = await clickCheckbox(
				"#ctl00_MainContent_CheckBoxList1_0",
				page,
				exceptionHandling
			)();
			if (!result) {
			...
			try {
				const element = (
					await page.$$(
						"label[for='ctl00_MainContent_RadioButtonList1_0'"
					)
				)[0];
				await page
					.evaluate((element) => {
				...
				console.log(
					"Error: List with dates is not available, will try later"
				);
				setEarliestDateCallback();
				await browser.close();
			}
...
const clickCheckbox = (selector, page, exceptionHandling) => {
	return async () => {
		...
		await Promise.all([element.click({ delay: 100 })]).catch((error) => {
			console.log(error);
			exceptionHandling(error);
			return false;
		});

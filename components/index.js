...
sendDailyReportWhenEvening = () => {
	...
	if (hour === 22 && minutes <= 30) {
		publish(
			"Daily report. Earliest date found today is [" +
				consoleOutDate(earliestDate) +
				"]"
		);
	}
};
...
setEarliestDate = (dateString) => {
	if (!dateString) {
		publish(
			"Unfortunately, there are no dates available at the moment. Will retry later."
		);
	...
	let receivedDate = new Date(
		`${stringTokens[2]}-${stringTokens[1]}-${stringTokens[0]}`
	);
	...
	if (datesNotTheSame(receivedDate, earliestDate)) {
		if (receivedDate < earliestDate) {
			...
			earliestDate = receivedDate;
			console.log(
				"New date is after saved earliest date.\r\nSomeone's picked it up. Shifting next available date to:\t" +
					consoleOutDate(earliestDate)
			);
		}
	} else {
		console.log(
			"Received date is later than saved earliest date. Do nothing."
		);
	}
	console.log("====  Stop iteration  ====");
};
...
handleWrongParsing = (number) => {
	console.log(
		"[ERROR] Quality of Image was bad, thus parsing failed [" +
			number +
